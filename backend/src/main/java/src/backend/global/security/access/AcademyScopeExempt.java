 ...
 * ERD §6.1 에 등재된 <b>37개 테이블</b>(직접 보유 17 + 부모 경유 20)의 저장소 메서드에 대해
 * "학원 조건"(부모 조인 포함) 아니면 이 애너테이션 중 하나를 요구한다. 주석은 지워도 아무도 모르지만,
 * 이 애너테이션이 없는 새 조회는 테스트가 실패시킨다. 즉 예외가 조용히 생겨날 수 없다.
 *
 * <p>반대 방향도 막는다 — 로그인·재발급·복구 조회는 학원 소속을 <b>아직 모르는 상태에서 시작</b>해
 * 좁힐 수단 자체가 없다. 다음 사람이 "격리를 빠뜨렸다" 로 오인해 학원 조건을 넣으면 로그인이
 * 통째로 실패하므로, 같은 테스트가 그 6개 조회에 대해 애너테이션 유지와 조건 부재를 함께 고정한다.
 */
