...
/**
 * 구성원(계정·역할) 등록·수정·삭제·비밀번호 초기화 권한.
 *
 * <p>필요 권한 {@code member:manage}. 현재 부여 역할은 학원 관리자·플랫폼 관리자이지만,
 * <b>어느 역할이 이 권한을 갖는지는 {@link RolePermissions} 한 곳에서만 정한다</b> —
 * 그래서 이 애너테이션에는 역할 이름이 없고, 새 역할이 생겨도 이 파일과 컨트롤러는 바뀌지 않는다.
 *
 * <p>어느 학원의 구성원인지는 서비스 계층의 TenantGuard 가 따로 본다.
 *
...
@Documented
@PreAuthorize("hasAuthority('" + Permissions.MEMBER_MANAGE + "')")
public @interface CanManageMembers {
}
