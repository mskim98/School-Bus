 ...
 *
 * <p>{@code elapsedSecondsSinceRaised} 는 DB 컬럼이 아니라 <b>조회 시점</b>에 {@code Clock} 으로
 * 계산한 파생값이다(목표 11 요구 — 저장하면 조회할 때마다 갱신 배치가 따로 필요해진다). 취소된
 * 신고는 더 흐르지 않도록 {@code canceledAt} 을, 확인된 신고는 {@code ackedAt} 을 종료 시점으로
 * 우선한다 — 그렇지 않으면 이미 끝난 신고의 경과 시간이 콘솔을 열어 둔 만큼 계속 늘어난다.
 *
...
public record AdminEmergencyItemResponse(Long id, AcademyInfo academy, Long runId, String busNo, String type,
        ...
        Long elapsedSecondsSinceRaised) {
