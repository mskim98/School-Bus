 ...
 *
 * <p>{@link src.backend.run.event.RunRouteConfirmedEvent} 와 같은 형태의 평범한 record 이며
 * {@code ApplicationEventPublisher.publishEvent} 로 곧바로 발행한다. {@code ChangeRequestStore} 의
 * 저장 트랜잭션 안에서 발행되므로, 저장이 롤백되면 이 이벤트도 함께 취소된다.
 *
