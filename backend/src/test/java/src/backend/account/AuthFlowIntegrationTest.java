...
class AuthFlowIntegrationTest {
     ...
     *
     * <p>거부측은 허용 밖 실제 핸들러 <b>14개 전부</b>다({@code AccountStatusGateEndpoints}) — 정본의
     ...
     * Phase 3 목표 6). 개수를 채우려고 엔드포인트를 새로 만들지 않는다.
     */
    @Test
    ...
    void 거부측_목록이_허용_목록_밖_실제_핸들러_전부와_일치한다() {
        ...
        assertThat(all)
                .as("프로덕션 핸들러 24개 — 늘었는데 이 단언만 고치면 거부측 목록이 낡는다. Phase 2 의 12 + Phase 3 의 12")
                .hasSize(24);
     ...
     * 13개를 먼저 두드리는 것도 같은 이유다. 상태가 {@code pending} 으로 돌아간 뒤에 두드리면 거부
     ...
     *
     * <p>실패를 그대로 던지지 않고 <b>어느 엔드포인트</b>인지 붙여 다시 던진다 — 14개를 한 루프로 도는
