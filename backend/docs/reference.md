...
형태 2가지 — 의존 없으면 `public final class` + `static`(`bus/access/BusCrewGuard`), Repository가 필요하면 스프링 빈(`student/access/GuardianAccess`). `global/security/access/AcademyScope`는 도메인 무관이라 `global`에 있는 예외(static).
...
`access/`와 달리 `global/` 아래인 이유: `access/`는 "이 사용자가 **이 리소스**를 만질 수 있는가"라서 판정 대상 엔티티를 소유한 도메인에 속하지만, `authz/`의 판정 대상은 특정 도메인이 아니라 **인가 어휘 자체**다. 어떤 도메인 Repository에도 의존하지 않고 14개 도메인 컨트롤러가 전부 참조하므로 `global/security/access/AcademyScope`와 같은 근거로 `global`에 둔다.
...
## 19. 설명 주석 — 클래스 · 메서드 · 기능마다

**모든 클래스 · public 메서드 · enum · 이벤트 · 포트 인터페이스에 설명 주석을 붙인다.** 기본은 **한 문장**이고, 두 번째 문장은 **첫 문장과 다른 질문에 답할 때만** 허용한다.

**판정 기준은 문장 수가 아니라 "두 문장이 같은 질문에 답하는가" 다.**

| 두 번째 문장이 답하는 것 | 판정 |
| --- | --- |
| 첫 문장과 **같은 질문**(무엇을 담당하는가)을 부연 | **줄인다.** 한 문장으로 안 줄면 책임이 둘이라는 신호 (§20.1) |
| **다른 질문** — 왜 이 형태인가 · 언제 교체·재작성되는가 · 무엇을 하지 **않는가** | **유지** |

세 문장 이상이면 분리를 검토한다. **테스트 클래스는 예외** — 왜 이 패키지에 있는가·왜 이 방식으로 검증하는가 같은 비직관적 결정은 문단으로 적어도 된다. 테스트는 의도를 모르는 사람이 고치면 검증력이 조용히 사라진다.

⚠ **리뷰에서 문장 수를 세어 결함으로 매기지 않는다.** 위 질문 기준으로 판정한다.

| 대상 | 붙이는 것 | 형식 |
| --- | --- | --- |
| 클래스 · 인터페이스 | **무엇을 담당하는가** + 왜 이 계층에 있는가 | 클래스 선언 위 `/** … */` |
| public 메서드 | **무엇을 보장하는가** — 입력·출력이 아니라 사후 조건 | 메서드 위 `/** … */` |
| 상태 전이 메서드 | 허용 전이와 거절 조건 | `/** … 아니면 {@link BusinessException}. */` |
| enum 상수 | 그 값이 뜻하는 실제 상황 | 상수 오른쪽 줄 주석 또는 `/** … */` |
| 이벤트 | **언제 발행되는가** (커밋 시점 기준) | 레코드 선언 위 |
| 포트(`spec`) | 교체 축이 무엇인가 · 구현체 후보 | 인터페이스 위 |
...
두 예시 모두 **두 문장이지만 각각 다른 질문에 답한다** — 앞 문장은 "무엇을 담당하는가", 뒤 문장은 각각 "왜 인터페이스인가" 와 "무엇을 보장하지 않는가". 아래는 줄여야 하는 형태다.

```java
// ❌ 두 문장이 같은 질문을 두 번 답한다
/** 학생과 보호자를 연결한다. 보호자가 자녀를 등록할 때 쓰인다. */

// ✅ 한 문장으로 줄이거나
/** 보호자가 자녀를 등록할 때 학생↔보호자 연결을 만든다. */
```
