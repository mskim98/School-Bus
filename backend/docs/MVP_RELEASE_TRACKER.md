...
> **진행 상황(2026-07-22 갱신):** P0(G2·G3·G1)·P(MVP)(F2·F4·F1·F3)·P1(G5)·P2(G6) 전부 완료 — **§2 백로그(MVP 필수 갭) 전체 완료.**
G4는 폐기 확정(사용자 확정, 아래 §1·§2 참조). F2/F4/F1/F3/G1~G6 전부 `main`에 커밋 완료(상세는 `git log`). §2 완료 이후
진행한 Swagger 태그 정리·"00. MVP 사용 API" 그룹·로컬 DB 재현성 정비도 커밋 완료(`88b2a73`).
...
> 전체에 단위테스트 추가, `./gradlew test` 97/97 통과(Docker 기동 상태 확인).
>
> **G4 폐기(2026-07-22, 사용자 확정):** 오프라인 태그(QR/NFC) 미도입·위치 전면 GPS 기반 확정으로 오프라인 동기화 문제 자체가
> 발생하지 않음. 차량고장 대응은 기존 `sos` 모듈(범용 SOS)로 충분해 전용 플로우 분리 불필요.
>
> **🎉 G6 완료 — MVP §2 백로그 전체 완료(2026-07-22):** NCP 실키 E2E 검증 과정에서 실키 없이는 드러나지 않던 버그 3건
...
| ~~**G4**~~ | ~~§6 태그/단말기 오류 오프라인 처리·차량고장 전용 대응~~ | **2026-07-22 폐기(사용자 확정)** — 오프라인 태그(QR/NFC) 자체를 도입하지 않고 위치는 전부 GPS 기반으로 확정. 차량고장 대응은 기존 `sos` 모듈(범용 SOS 발신·에스컬레이션)로 충분하다고 판단해 전용 플로우 분리 불필요 — G4 항목 전체 폐기 |
...
**우선순위 (사용자 확정, 2026-07-21): 안전 기능(G2·G3·G1) → 테스트(G5) → 나머지(G4·G6)** — G2·G3·G1·G5 모두 완료(2026-07-22). G4는
같은 날 폐기 확정(오프라인 태그 미도입·GPS 전면 채택, 차량고장은 기존 sos로 충분). **남은 건 G6뿐.**
...
- [x] **F3 — 노선 배포 시 기사 알림(publish 트리거 추가)** (2026-07-21 완료, 커밋 완료): 알림 파이프라인이 studentId 기준이라 배포 알림의
...
- ~~**G4 — 오프라인 태그 동기화·차량고장 전용 플로우**~~ **(2026-07-22 폐기, 사용자 확정)**: 오프라인 태그(QR/NFC) 자체를 도입하지
  않고 위치는 전부 GPS 기반으로 확정 — 오프라인 동기화 문제 자체가 발생하지 않음. 차량고장 대응은 기존 `sos` 모듈(범용 SOS 발신·
  에스컬레이션)로 충분하다고 판단해 전용 플로우 분리 없이 그대로 유지.
- [x] **G6 — NCP 실키 E2E 검증** (완료, 2026-07-22): 실키로 `NaverMapRouteClient`를 검증하며 실키 없이는 드러나지 않던 버그
  3건을 발견·수정했다.
  1. **URI 인코딩**: waypoints 구분자 `'|'`를 `URI.create()`로 직접 파싱하면 `URISyntaxException`(URI 예약문자가 아님) —
     `UriComponentsBuilder`+`.encode()`로 percent-encoding하도록 교체.
  2. **응답 스키마 불일치**: `option` 쿼리파라미터를 명시하지 않으면 NCP 기본값이 `traoptimal`이라 코드가 기대하는 `trafast`
     키가 응답에 없어 파싱 실패 — `option=trafast` 명시.
  3. **연속 중복 좌표 거부**: 같은 정류장에 배정된 학생이 인접하면 좌표가 연속 중복되는데, NCP가 이를 "출발지와 도착지가
     동일합니다" 400으로 거부한다 — 호출 전 연속 중복 좌표를 접어서 보내고, 접힌 구간은 실제 이동거리가 0이므로 leg
     duration도 0으로 되살리는 로직을 `NaverMapRouteClient.route()`에 추가(인덱스는 원래 waypoint 개수와 항상 일치).
  <br>NCP 콘솔 쪽 문제도 있었다(사용자가 확인·해결) — 이 키가 속한 Application에 Directions API가 활성화돼 있지 않아 403
  (엔드포인트까지는 도달했으나 거부됨·응답 바디 없음·NCP 콘솔 사용량 미반영으로 진단) → 콘솔에서 활성화 후 정상화.
  <br>**waypoint 실측 한도 확정**: NCP `waypoints`(경유지만) 상한은 5개, start+goal 포함 총 7개가 API 1회 호출당 한도임을
  실측으로 확인(기존 코드 주석의 "총합 5개" 추정을 정정). `routing.max-waypoints`를 15→7로 수정(코드 `@Value` fallback
  기본값도 동일하게 조정).
  <br>**부수 개선**: `GlobalExceptionHandler.handleUnexpected`가 예외를 완전히 삼켜(로그 없음) 원인 추적이 불가능했던 문제를
  발견 — 서버 로그에 스택트레이스를 남기도록 수정(클라이언트 응답은 기존과 동일하게 상세 비노출 유지).
  <br>**검증**: 실키를 `.env`에서 export해 기동 → ① bus1 PICKUP(7 waypoint, 같은 정류장 학생 중복 포함) 200 확인 ② 임시
  학생 3명을 SQL로 추가해 10 waypoint로 늘려 앱 레벨 청킹(2회 실 API 호출로 분할 후 병합) 200 확인, 이후 임시 데이터 원복
  ③ `application.yml`에 체크인된 최종 설정(`routing.provider=naver`, `routing.max-waypoints=7`)만으로, 수동 env override
  없이 재기동해 동일 케이스 재확인(파일 설정만으로 정상 동작 확인) ④ `./gradlew test` 97/97 유지(회귀 없음).
  <br>**최종 상태**: `routing.provider` 기본값을 `naver`로 전환 완료(2026-07-21 계획 단계에 사용자 확정). **G6 완료로 §2
  백로그 전체(P(MVP)·P0·P1·P2) 완료.**
