...
> **진행 상황(2026-07-22 갱신):** P0(G2·G3·G1)·P(MVP)(F2·F4·F1·F3)·P1(G5) 전부 `main`에 커밋 완료(상세는 `git log`).
G4는 폐기 확정(사용자 확정, 아래 §1·§2 참조). **남은 건 G6뿐.**
...
> 발생하지 않음. 차량고장 대응은 기존 `sos` 모듈(범용 SOS)로 충분해 전용 플로우 분리 불필요. **P2에 G6 하나만 남음 — G6
> 완료 시 `MVP_RELEASE_TRACKER.md` §2 백로그 전체(P(MVP)·P0·P1·P2) 완료.** (§3 업그레이드 로드맵은 MVP 범위 밖, 별도 트랙)
