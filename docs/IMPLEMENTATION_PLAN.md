...
| **8** | 탑승 의사 · 변경 요청 · 3구간 승인 · 관계자 노선 조작 | 7 | P0/P1 | 🟡 | X-02 · **착수 2026-08-30**, 분기점 `4da464e`. 목표 **18항**(완료 조건 12 + Phase 7 이월 5 + 전체 실측 1) — 표는 `p8-goal-table.md`. **착수 전 판정 Ruling 195~199** 로 완료 조건 4항을 갈랐다(WS 방송 → Phase 10 · start 배선 → Phase 9 · `§5.8 transfer` → 별도 단위 · ①구간 재최적화 문면은 정본 모순이라 개정). 신설 핸들러 예상 **8개**(분기점 56) |
