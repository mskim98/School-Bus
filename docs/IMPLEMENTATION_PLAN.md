...
⚠ **Phase 2 에서 이월 (2026-08-25, Ruling 117)** — **매니저 → `Assignment` 범위 축소**. Phase 2 Task 5 가 학원 격리는 완성했으나 역할별 추가 축소는 미착수. 사유는 재료 부재 — `Assignment` 엔티티는 있으나 저장소가 없고 판정 대상인 **회차 조회 경로가 이 Phase 의 산출물**이다. 소비자를 모르는 채 시그니처를 정하면 추측이 된다. **회차 조회를 만들면서 함께 만든다.**

### Phase 9 — 운행 실행 · 승하차 · 명단

🛑 **착수 전 사용자 확인이 필요하다 (2026-08-26 사용자 지시).** `RUN-08`(외부 내비 앱 연동)의 **앱별 경유지 개수 상한**(오픈 이슈 **V**)이 미실측이다. **이 Phase 를 시작하기 전에 사용자에게 알리고 값을 받는다.** 값 없이 `scope=remaining` 을 만들면 앱이 조용히 잘라 **기사가 다른 경로로 간다** — 오작동이 아니라 안전 문제다. `scope=next` 는 상한과 무관해 값이 없어도 진행 가능.
...
**외부 내비게이션 앱 연동** (`RUN-08` · `API_SPEC §4.16`, **2026-08-26 사용자 요청으로 신설**) — 확정 노선을 티맵·카카오내비로 넘길 **좌표열**을 반환한다. **서버는 딥링크를 만들지 않고** 순서·제외·자르기만 판정하며, 조립은 앱이 한다(근거는 `§4.16`).
...
⚠ **막히는 지점** — 티맵·카카오내비의 **경유지 개수 상한**이 미확인 (오픈 이슈 **V**). 상한을 모르면 `scope=remaining` 이 앱에서 조용히 잘려 **기사가 다른 경로로 간다.** 실측 전까지 `scope=next` 만 신뢰 대상이고, 완료 조건도 그렇게 적는다.
...
- **(RUN-08)** `GET /runs/{runId}/navigation?provider=tmap&scope=next` 가 **다음 미도착 승하차지**의 좌표를 반환하고, `arrived_at` 이 찍힌 지점과 `skipped` 지점은 **결과에 등장하지 않는다**
- **(RUN-08)** 남은 승하차지가 0개면 `409 NAV_NO_REMAINING_STOP` — 빈 배열을 200 으로 반환하지 않는다
- **(RUN-08)** `scope=remaining` 에서 남은 지점이 앱 상한을 넘으면 **잘라서 반환하고 `truncated=true`** 이며 `total_remaining_stops` 가 자르기 **전** 개수를 담는다
...
| **5** | 기초 데이터 — 학생 · 보호자 · 주소 · 차량 · 인력 · 스케줄 | 3 | P0(유도) | 🟡 | **2026-08-26 착수 — Phase 4 와 병렬**(§8.1 확정). 목표 **12항** 고정(`p5-goal-table.md`, Ruling 159 로 12 보충). 오픈 이슈 **L·K·M 을 착수 전 판정**(Ruling 149~151) · 정본 충돌 3건 정정(Ruling 152·154·155) · `[조정 중]` §5.10·§5.14 확정(Ruling 153). **회수 4/7 태스크** — T1 학생 `482dfed` · T2 차량·매니저 `319dc63` · T3 자녀 연결 `844ea62` · T5 스케줄·회차·배치 `e670a3f`. **통합 트리 단독 실측(2026-08-27, 신규 DB) 95클래스 586테스트 실패 0 · 오류 0** · 프로덕션 핸들러 **48**(직접 셈). **통과 목표 6항**(2·3·4·5·7·8). **미통과 6항** — 1(흐름 완주) · 6(요일별 주소) · 9(`role='staff'` 판정) · 10(픽스처 raw SQL) · 11(게이트 총계 확정) · 12(사진 업로드). **잔여 태스크 3개, 브리프 작성 완료** — T4(`p5-task-4-brief.md`) · T6(`p5-task-6-brief.md`) · T7 종단(`p5-task-7-brief.md`). 셋 다 **분기점 해시 미기재** — 조율자가 발주 시 채운다 |
...
| **V** 🆕 🛑 | **외부 내비 앱의 경유지 개수 상한** (티맵 · 카카오내비) — **Phase 9 착수 전 사용자에게 알릴 것**(2026-08-26 지시) | **9** | `scope=next`(다음 1개)로는 착수·완료 가능. **`scope=remaining` 은 상한 실측 전까지 완료 판정 불가** — 모르면 앱이 조용히 잘라 기사가 다른 경로로 간다. 값이 정해지면 코드 상수로 두고(규칙 10) `API_SPEC §4.16` 에 적는다 (2026-08-26 신설, RUN-08) |
