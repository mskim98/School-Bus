...
매 Phase 착수 전·완료 전에 훑는 체크리스트. 근거는 `TECH_DECISIONS` (규칙 19·20 은 `backend/docs/reference.md`, 규칙 21·22 는 §4.6·§4.7). **22개 전부 백엔드 규칙**이고 프론트 전용 항목은 부재 — 프론트 전용 규칙은 Phase F1~F4 절의 "전 제품 공통 3가지"에 있으며 그 절은 현재 ➖ 범위 밖.
