...
| **3** | 학원 · 관계자 승인 · 메인 관리자 콘솔 기초 | 2 | P0 | **🟡** | 착수 2026-08-26. 목표 표 `p3-goal-table.md` **7항**(정본 5 + 이월 2). 오픈 이슈 **R·S·T 착수 시 해소**(Ruling 140 학원 코드 난수 8자 · `§6.3` soft delete 만 · `C-01` 로 T 전제 소멸). ⚠ **차단 결함 1건 발견** — `uk_academy_staff_academy` 가 전체 UNIQUE 라 관계자 교체가 영구 불가(Ruling 139, partial UNIQUE 로 정정). **Phase 2 이월 2건** — 계정 상태 게이트 왕복 테스트의 거부측을 새 엔드포인트로 확대(Ruling 133) · `blocked` 전이·권한 회수 시 refresh 토큰 전량 무효화(그러지 않으면 `RefreshCommandService` 가 `assertNotBlocked()` 를 생략한 근거가 무너짐) |
| **4** | 알림 아웃박스 골격 | 3 | P0 | ⬜ | |
