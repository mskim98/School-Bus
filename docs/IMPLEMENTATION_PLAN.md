...
| **9** | 운행 실행 · 승하차 · 명단 | 8 | P0 | 🟦 **착수** | **착수 전 확인 완료(2026-08-31)** — 사용자가 미결 5건을 닫았다: **A** 등원 자동 하차도 알림 발송 · **J** 되돌리기는 취소 동작까지만(허용 범위·알림 정정은 Phase 12) · **X-01** 운행 시작 창 **±3분 → ±10분** + `confirmed` 시점부터 노선 열람 · **X-03** 동승자 부재 회차는 없다고 가정 · **V** **MVP 카카오내비 단독 · 상한 승하차지 4곳**(티맵은 스킴 규격 미공개라 미구현, 어댑터 자리만). 판정은 **Ruling 201~205**. 완료 조건 **21항** + 전체 실측 1 = **22항**, 목표 표는 `.superpowers/sdd/IMPLEMENTATION_PLAN/p9-goal-table.md`(표 행 23 — 하원 자동 승차를 항목으로 세움). 분기점 **`d430f4c`** · 실측 핸들러 **65** · 컨트롤러 25 · 테스트 클래스 149 · 테이블 40. 신설 핸들러 **10개 예상**(65 → 75). 좌석 **4개 병렬** — T1 조회·명단 / T2 운행 시작·종료 / T3 승하차·되돌리기 / T4 내비. 이월 4건 포함(Ruling 117·148·192·196·199) |
