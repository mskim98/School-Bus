...
| 23 | **음성 대조로 단언을 실측** — 새 단언마다 그것이 잡으려는 결함을 프로덕션에 심어 **그 단언만** 실패하는지 확인. 원복 후 `git status --porcelain` 이 빈 것까지 | `phase-goal-loop §5` | 초록이 "결함 부재" 가 아니라 **"이 스위트가 결함을 못 봄"** 과 구분되지 않음 |
...
| **2** | 인증 · 계정 상태 게이트 · RBAC · 학원 격리 | 1 | P0 | **✅** | 완료 조건 **11항 전부 실증**(2026-08-26, 근거 `p2-completion-evidence.md`). 스위트 **55클래스 298테스트 실패 0**. 태스크 6개 + 수정 라운드 5회 + 게이트 리뷰 5회. **음성 대조 19건으로 검증 구멍 6건 발견·해소.** 이월 — ①조건 2·3 의 거부측 목록을 Phase 3 이 엔드포인트를 늘릴 때 함께 확대(Ruling 133) ②로그인 시도 빈도 제한 → Phase 14(Ruling 127·137) ③학부모·매니저 축소 → Phase 5·9(Ruling 117) ④STOMP 축 계정 상태 게이트 · `/topic/tenant/{id}` 경로 어휘 → Phase 10(Ruling 87·121) |
| **3** | 학원 · 관계자 승인 · 메인 관리자 콘솔 기초 | 2 | P0 | ⬜ | 오픈 이슈 R·S. **Phase 2 이월 2건** — 계정 상태 게이트 왕복 테스트의 거부측을 새 엔드포인트로 확대(Ruling 133) · 관리자 차단(`blocked` 전이) 시 refresh 토큰 전량 무효화(그러지 않으면 `RefreshCommandService` 가 `assertNotBlocked()` 를 생략한 근거가 무너짐) |
| **4** | 알림 아웃박스 골격 | 3 | P0 | ⬜ | |
