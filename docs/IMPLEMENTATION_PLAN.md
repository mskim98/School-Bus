...
| **V** 🆕 | **외부 내비 앱의 경유지 개수 상한** (티맵 · 카카오내비) | **9** | `scope=next`(다음 1개)로는 착수·완료 가능. **`scope=remaining` 은 상한 실측 전까지 완료 판정 불가** — 모르면 앱이 조용히 잘라 기사가 다른 경로로 간다. 값이 정해지면 코드 상수로 두고(규칙 10) `API_SPEC §4.16` 에 적는다 (2026-08-26 신설, RUN-08) |
