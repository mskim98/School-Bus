...
| **210** | **근접 판정 스케줄러의 중복 실행 방지는 조건부 UPDATE 선점**(Phase 7 확정 배치의 선례)으로 한다. `ARCHITECTURE §9` 가 분산 락을 요구하나 저장소에 락 수단이 부재하고, 새 의존성(ShedLock)을 들이는 것보다 **이미 음성 대조까지 끝난 선례를 재사용**하는 편이 검증 비용이 낮다 | 이 절 |
