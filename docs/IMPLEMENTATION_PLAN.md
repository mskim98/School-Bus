...
|---|---|:-:|---|
| **L** | 주소 검증 수단 (지오코딩 API) | ~~5~~ → **6** | **2026-08-26 Ruling 149** — `GeocodingClient` 포트 + 결정론적 스텁으로 Phase 5 완료 조건을 판정한다. 실 API 선정은 Resilience4j 보호와 함께 **Phase 6** 이 받는다 |
| ~~K~~ | 매니저 근무시간 자료형 | **해소 — 착수를 막지 않음** | **2026-08-26 Ruling 150** — `ERD manager.work_hours` 가 이미 확정 문면(구조화된 시간 범위, 2026-08-24)이었다. jsonb 안의 형태(요일 키 × 구간 배열 × `HH:mm`)를 Ruling 150 이 고정 |
| **R** | 학원 코드 형식 규칙 | **3** | `varchar(32)` + UNIQUE 로 진행 가능. 자동 생성 여부만 미결 |
| ~~M~~ | 학생 주소 수정 반영 시점 (즉시 / 익일) | **해소 — 즉시 반영** | **2026-08-26 Ruling 151** — `weekly_address` 에 발효일 컬럼이 부재하고, 일일 변경(REQ)이 이미 "그날만" 을 담당한다. 확정된 회차로의 소급 부재는 Phase 7·8 소유 |
