...
| `Path=/api/v1/auth` | 쿠키가 붙는 요청을 3개 엔드포인트로 한정 |
...
| 운행 시작 | `status='confirmed'` → `moving` (±10분 창 검사와 함께) |
...
이 시스템은 **시각이 상태를 만든다** (`ARCHITECTURE §1.3`). 검증해야 할 규칙이 전부 시각 의존이다 — 출발 30분 전 도래, ±10분 창, 두 시계 분리, ②구간 마감, 3분 카운트다운.
...
| 운행 시작 ±10분 창을 놓침 | **해소 (X-01, Ruling 202)** — 창을 ±10분으로 넓혀 예외 절차 자체를 두지 않음 |  |
