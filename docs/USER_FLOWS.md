...
| 5 | (자동, 5초마다) | — | `POST /api/locations/bus`<br>`{busId, lat, lng}` | 서버가 `RedisBusLocationRepository`(키 `busloc:{busId}`, **TTL 15초**)에 **버스당 최신 1건만** 덮어쓴다. `tenantId` 는 `bus.tenant` 에서 파생, `origin` 은 **앱의 소스 토글값**(`GPS`/`MOCK`)을 그대로 저장 |
...
- **기사 앱이 보낸 좌표는 `origin` 이 앱의 토글을 따른다**(2026-08-21~). Mock 토글이면 `MOCK`, 실 GPS면 `GPS` 를 실어 보내고 서버가 그대로 저장한다. 관제 상세 패널의 "출처: 단말/시뮬레이터" 표기가 실제를 반영한다. ⚠ 단 이는 **단말 자기신고**라 서버가 검증할 수단은 부재하며, 서버 Mock 소스가 덮어쓰면 그쪽 `MOCK` 이 남는다.
...
| C15 | ~~**버스 마커가 진짜 기사인지 서버 시뮬레이터인지 화면에서 알 수 없다**~~ → ✅ **해소**(2026-08-21, MON-2) | — | 기사 앱이 `origin` 에 자기 소스 토글값을 실어 보내고 서버가 그대로 기록한다. 관제 화면이 `origin` 으로 "단말/시뮬레이터"를 구분한다. ⚠ 잔존: 서버 `MockBusLocationSource` 와 기사 앱이 **여전히 같은 저장소를 서로 덮어쓴다** — 실 운행에서는 `bus-mock.enabled=false` 로 꺼야 한다 (A3·A4) |
