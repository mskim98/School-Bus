...
### 다음에 할 일 — **브라우저 육안 점검 하나뿐이다**

코드·문서·감사·테스트는 전부 끝났다. 남은 건 **사람 눈으로만 할 수 있는 것**이다.

배포는 확인돼 있다(2026-07-29):

```
컨테이너 6개 기동 · http://localhost/ 200
main.dart.js md5 — 컨테이너 30e203a2… == 서빙 30e203a2…   (프록시가 옛 파일을 물고 있지 않다)
번들에 D9 수정분 문자열 존재 확인 ('배차 화면으로' 등)
```

⚠️ **그래도 옛 화면이 보이면 서버가 아니라 브라우저다.** 위 md5 가 일치하므로 남은 변수는 service worker 뿐이다. **시크릿 창**으로 열어보는 게 가장 빠른 판별법이고, 일반 창이면 `F12 → Application → Service Workers → Unregister → Clear site data`. `Cmd+Shift+R` 은 SW 를 우회하지 못한다(`FLUTTER_FRONTEND_PLAN.md` §9).

**볼 순서** (`driver@school.com` / `admin@school.com`, 비밀번호 전부 `password`)

| # | 화면 | 무엇을 확인하나 |
|---|---|---|
| 1 | 로그인 | 빠른 로그인 패널이 있는 레이아웃이 자연스러운가 |
| 2 | 운행 시작 | 등원/하원 카드에 **기본 선택이 없는가**, 선택 시 색 말고 **체크 아이콘**도 바뀌는가, 시작 버튼이 56dp 로 큰가 |
| 3 | 오늘의 노선 | 지도에 경로선(굵기 9)·정차 마커·버스가 뜨는가, **진하게 튀는 정차 카드가 하나뿐인가**, ETA 가 `출발`/`N분 후` 로 나오는가 |
| 4 | 승하차 명단 ★ | 김민준·이서연·박도윤이 뜨는가, 기록 시 **전송 중 → 확정** 2단계로 가는가(즉시 확정되면 회귀), 하차(주황)와 인계(초록)가 구분되는가 |
| 5 | 운행 종료 | 잔류 학생이 있을 때 **종료가 막히고 이유가 보이는가** |
| 6 | 관리자 관제·배차·노선·알림 | 알림함에서 **하차와 인계가 다른 색인가**(H1 회귀 지점), 빈 노선 목록에 `배차 화면으로` 버튼이 있는가 |
| 7 | 라이트/다크 | OS 테마를 바꿔 6개 화면 재확인 — 대비가 무너지는 곳 |
| 8 | 글자 200% | OS 글자 크기를 키워 명단·노선 요약이 안 잘리는가 |

본 결과는 아래 §지시서 수용 기준 표에 기록한다.
